...
async def test_handler_returns_response():
    ...
    with patch("airbnb_travel_agent.main._initialized", True), \
         patch("airbnb_travel_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response):
...
async def test_handler_with_multiple_messages():
    ...
    with patch("airbnb_travel_agent.main._initialized", True), \
         patch("airbnb_travel_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("airbnb_travel_agent.main._initialized", False), \
